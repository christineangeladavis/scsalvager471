...
- **Aggregated incoming user mail** (added 2026-05-09):
  - `GET /api/admin/inbox-overview` (admin-only). Iterates `listUserIds()`, reads each `inbox:<userId>`, returns every entry where `from === "user"` and `deletedAt` is unset AND the id is not in this admin's per-admin dismissed set, flattened with `userId` + `username` attached. Cap **200** newest. Sorted desc by `createdAt`.
  - `POST /api/admin/inbox-overview { action: "dismiss", id }` (admin-only). Adds the entry id to `admin-overview-dismissed:<adminUserId>` (cap **500**). Hides the entry from THIS admin's overview only — the entry stays in the user's inbox, still appears in the user-detail thread, still visible to other admins.
  - Recipient surface: admin's Messages mailbox dropdown gains a top **User mail · N** section (amber-tinted) above the regular Messages list. Each entry shows username + body preview + timestamp; clicking jumps to that user's user-detail modal with the full thread pre-loaded. Each row also has a per-admin **Delete** button that calls the dismiss endpoint optimistically — the entry vanishes from the overview but stays in the user's thread.
