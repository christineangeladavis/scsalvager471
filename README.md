...
- New **Inventory** sub-tab on the Ledger surface (right of Crew Salvage). Card grid where each storage location renders as a rounded square with side-by-side **CMAT** + **RMC** SCU pills + last-activity timestamp. Cards sort by combined SCU descending. Click any card → modal with current totals + a live `+ → / − →` projection as you type SCU; **Add** (emerald) or **Remove** (rose) buttons. Inline **Add to Inventory** panel above the grid for fast deposits without picking a row first. Storage locations grouped Stanton / Pyro / Nyx (cities, orbital stations, Lagrange stations, jump gateways, plus 4 Levski People's Service Stations). Backed by sell-orders with location prefix `Send to Local Storage · X` and `aUEC: 0` so deposits don't move lifetime aUEC. Withdrawals blocked when they'd drive a (location, material) bucket below zero. Gated on the patch 4.8 live release.
- New **Origin M80** pre-staged in the Crew Salvage > Ships Salvaged picker (gated on the 4.8 live release). Display names cleaned up across all 7 pre-staged ships — Drake / Aegis / Kruger / MISC manufacturer prefixes dropped (now Ironclad, Ironclad Assault, Pitbull, Tiburon, Stingray, Starlite, M80). Manufacturer field still set per ship; Starlite alignment fixed to "Musashi Industrial and Starflight Concern" (matches SPViewer dataset).
...
- On 4.8: form labels site-wide turn **yellow** (Missions filter row, Refinery & Sell Orders forms, Refinery Bonus Yield Calculator, Sell Estimate). On 4.7.2 the labels stay slate as today.
- On 4.8: mission detail popup briefing collapses long `[LOCATION]` / `[DESTINATION]` token expansions (4+ resolved entries) into a hover popover so refueling missions with 100+ pickup chips don't blow out the modal. Pickup + Destinations chip stack uses the same threshold. Short lists (1–3) stay inline. On 4.7.2 the chips render inline regardless of count.
- Mission detail dataset refresh from the latest scmdb 4.8 PTU dump (`4.8.0-ptu.11777743`): refueling missions 13 → 14 (one new contract added; all 13 prior contracts got reward + time-to-complete bumps), salvage blueprint pools regenerated, and scraper-module fabricator craft times updated (Abrade 90s → 240s, Trawler 90s → 300s). Visible only on the patch 4.8 live release.
