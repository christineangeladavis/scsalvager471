...
- **Ledger tab → Inventory sub-tab — three more materials** tracked: **Construction Salvage**, **Construction Pieces**, **Construction Rubble**. The five-material set (CMAT, RMC, Salvage, Pieces, Rubble) lets you stockpile raw refinery inputs at the same storage locations as the refined finished goods. Header chip strip + per-location card tiles + the per-location modal projection grid all show every material; cards only render tiles for materials with a non-zero balance so they don't bloat with empty rows.
- **Ledger tab → Crew Salvage — auto-credit registered pilots on session complete.** When you click **Mark Complete** on a Crew Salvage session, every crew slot whose roster name matches a registered SCSalvager user gets the same per-pilot split share (totalScu / headcount, splitAuec / headcount) appended to their own ledger — no captain-side coordination needed. Pilots who aren't registered are silently skipped; the captain still sees the full run on their own ledger as before.
- **Ledger tab → Crew Salvage — roster typeahead.** Start typing a name in any role slot and a dropdown of registered SCSalvager users whose display name contains what you've typed appears beneath the input. Click a suggestion to autofill the slot exactly — useful for catching typos that would otherwise skip a pilot's auto-credit on session complete.

Fixes:
- **Ledger tab → Patch History — past-patch entries re-surface after a patch-reset clear.** If you clicked "Clear ledger for new patch" on the 4.8 drop, every 4.7.2 entry got `deletedAt` stamped at clear time and the panel filtered them all out. Patch History now distinguishes patch-reset clears (deletion timestamp ≥ patch end) from in-cycle Discard clicks; past-patch views re-surface the patch-reset clears so you can look back at your 4.7.2 history. Mid-cycle discards stay hidden. Global Statistics + Lifetime aUEC pill are unaffected — they continue to filter soft-deleted entries server-side regardless of which view you're on.
...
- **SCSalvager Desktop** is now downloadable from **Settings → Desktop App**. Native client (Windows / macOS arm64 / Linux) wraps the site with a system tray, refinery countdown badge, OS toasts when a job is ready, tray-driven screenshot capture for the in-game refinery setup screen (uses the existing crop modal before upload), an offline read-only ledger cache, and silent auto-update on launch.
...
- **Statistics global-stats tiles** now autofit to the longest descriptor instead of stretching to a third of the viewport on wide screens. Tiles pack as many columns as the container allows; the layout still falls back to a single column on mobile / narrow windows.
- **Settings → Desktop App** descriptor dropped the F9 hotkey mention (Easy Anti-Cheat blocks it while Star Citizen is focused — tray capture is the reliable trigger). "Release notes →" now opens an in-app desktop changelog modal sourced from the new `DESKTOP_README.md` instead of bouncing out to a GitHub release page.
